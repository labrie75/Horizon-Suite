...
* **Dynamic Scanning** – Surfaces active Scenarios, Delves, and World Events in dedicated, high-priority sections. Includes support for cinematic progress bars, timers, and difficulty tiers. Delves appear in the quest list with scenario objectives and quests; season affixes can be shown on the first entry (Features → Scenario & Delve → Show affix names in Delves).
...
* **Delve Display** – When in a Delve, the quest list shows delve objectives and quests in the standard entry layout. Enable **Show affix names in Delves** (Features → Scenario & Delve) to display season affixes on the first Delve entry.
